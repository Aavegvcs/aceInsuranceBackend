...
@Entity('proposers_medical')
export class ProposersMedical extends BaseEntity {
    @PrimaryGeneratedColumn()
    id: number;

    @ManyToOne(() => InsuranceUser, (data) => data.medicalDetails, { nullable: true })
    @JoinColumn({ name: 'insurance_user_id' })
    insuranceUserId: InsuranceUser;

    @ManyToOne(() => InsuranceTicket,(data)=> data.proposerMedical,{ nullable: false })
