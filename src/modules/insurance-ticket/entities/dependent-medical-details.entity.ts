...
@Entity('dependent_medical')
export class DependentMedical extends BaseEntity {
    @PrimaryGeneratedColumn()
    id: number;

    @ManyToOne(() => InsuranceDependent, (data) => data.medicalDetails, { nullable: false })
    @JoinColumn({ name: 'dependent_id' })
    dependentId: InsuranceDependent;

    @ManyToOne(() => InsuranceTicket, (data)=>data.dependentMedical, { nullable: false })
